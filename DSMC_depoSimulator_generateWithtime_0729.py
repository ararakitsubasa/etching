...
class depo(transport):
    ...
    def depo_film(self, film, pos, vel, i, j, k, weights_arr, depoStep):
        ...
        try:
            indice_inject = np.array(film[i, j, k] >= 1)
        except IndexError:
        ...
        #etching
        surface_etching = np.logical_and(film >= 0, film < 1) # depo
        self.surface_etching_mirror[10:10+self.cellSizeX, 10:10+self.cellSizeY, :] = surface_etching
        self.surface_etching_mirror[:10, 10:10+self.cellSizeY, :] = surface_etching[-10:, :, :]
        self.surface_etching_mirror[-10:, 10:10+self.cellSizeY, :] = surface_etching[:10, :, :]
        self.surface_etching_mirror[10:10+self.cellSizeX, :10, :] = surface_etching[:, -10:, :]
        self.surface_etching_mirror[10:10+self.cellSizeX:, -10:, :] = surface_etching[:, :10, :]
        self.surface_etching_mirror[:10, :10, :] = surface_etching[-10:, -10:, :]
        self.surface_etching_mirror[:10, -10:, :] = surface_etching[-10:, :10, :]
        self.surface_etching_mirror[-10:, :10, :] = surface_etching[:10, -10:, :]
        self.surface_etching_mirror[-10:, -10:, :] = surface_etching[:10, :10, :]
        surface_tree = KDTree(np.argwhere(self.surface_etching_mirror == True)*self.celllength)
        dd, ii = surface_tree.query(pos_1, k=self.kdtreeN, workers=10)
