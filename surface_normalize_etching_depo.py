...
class surface_normal:
    def __init__(self, center_with_direction, range3D, InOrOut, celllength, fullCell, yield_hist = None):
        # center xyz inOrout
    ...
    def scanZetching(self, film): # fast scanZ
    ...
    def scanZdepo(self, film): # fast scanZ
        film = torch.Tensor(film)
        xshape, yshape, zshape = film.shape
        ...
        # 获取当前平面与前后平面的布尔索引
        current_plane = film < self.fullCell
    ...
    def get_pointcloud_Etching(self, film):
        test = self.scanZetching(film)
    ...
    def get_pointcloud_Depo(self, film):
        test = self.scanZdepo(film)
        points = test.indices().T
        surface_tree = KDTree(points)
        dd, ii = surface_tree.query(points, k=18, workers=5)
